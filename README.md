...
* A possible flow of steps will be to 
1. create your 
    * Components, 
    * action creators, 
    * reducers, 
...
[x] 2.  use the Provider component from `react-redux` to pass the store to your root component and 
...
4. use `redux-thunk` as a middleware inside of `src/index.js`. You'll want to be sure to pass it to `applyMiddleware()` then feed it into your createStore function.

5. * If you so choose, include `redux-logger` to your middleware. You're going to have plenty of action creators that will consume our API so you'll get plenty of actions triggered.
